...
echo "the following is the number of pending jobs of the user who has the most pending jobs, followed by the username, followed by the total nodes for said jobs"
...
echo "This is part 3, and I have a ton of people with 8-character usernames, so this is just the first one" 
...
echo "This is the number of nodes TOTAL being used by a single user, it prints the node count then the user for the top user"  
...
echo This is the other interpretation, namely who has the single job that needs the most nodes, and how many nodes  
cat queue.txt | tr -s ' ' |awk '{print $7,$4}'|sort -n -r | head -1

#and lo this is the last one, how many unique users have pending jobs because of priority
echo and lo this is the last one, how many unique users have pending jobs because of priority 
